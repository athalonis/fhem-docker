########################################################################################################################
# $Id: 57_SSCal.pm 21375 2020-03-08 08:40:57Z DS_Starter $
#########################################################################################################################
#       57_SSCal.pm
#
...
sub SSCal_setVersionInfo($) {
  ...
  if($modules{$type}{META}{x_prereqs_src} && !$hash->{HELPER}{MODMETAABSENT}) {
	  ...
	  if($modules{$type}{META}{x_version}) {                                          # {x_version} ( nur gesetzt wenn $Id: 57_SSCal.pm 21375 2020-03-08 08:40:57Z DS_Starter $ im Kopf komplett! vorhanden )
		  $modules{$type}{META}{x_version} =~ s/1\.1\.1/$v/g;
	  } else {
		  $modules{$type}{META}{x_version} = $v; 
	  }
	  return $@ unless (FHEM::Meta::SetInternals($hash));                             # FVERSION wird gesetzt ( nur gesetzt wenn $Id: 57_SSCal.pm 21375 2020-03-08 08:40:57Z DS_Starter $ im Kopf komplett! vorhanden )
